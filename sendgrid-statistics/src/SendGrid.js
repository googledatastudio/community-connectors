...
function getSendGridConfig(request) {
  ...
  config
    ...
    .setId('aggregated_by')
    .setHelpText(
      'How the statistics should be grouped on output. By day, week or month.'
    )
    .addOption(
      config
        .newOptionBuilder()
  ...
  config
    ...
    .setId('stats_type')
    .setHelpText(
      'Choose the email statistics type you would like to report on.'
    )
    .setIsDynamic(true)
    .addOption(
      config
    ...
    .addOption(
      config
        .newOptionBuilder()
        .setLabel('Mailbox Provider')
        .setValue('mailbox_providers')
    )
    .addOption(
      config
        .newOptionBuilder()
    ...
    .addOption(
      config
        .newOptionBuilder()
        .setLabel('Country and State/Province')
        .setValue('geo')
    )
    .addOption(
      config
        .newOptionBuilder()
        .setLabel('Device Type')
        .setValue('devices')
    )
    .addOption(
      config
        .newOptionBuilder()
        .setLabel('Client Type')
        .setValue('clients')
    );
    ...
    switch (request.configParams.stats_type) {
      ...
      case 'browsers': {
        ...
        config
          ...
          .setHelpText(
            '[Optional] Enter the items you would like to include - one line per item. Leave blank to return all items (see SendGrid API docs)'
          )
          .setPlaceholder(
      ...
      case 'geo': {
        config
          ...
          .setAllowOverride(true)
          .setHelpText(
            '[Optional] Select a country from the list or leave blank to return all values.'
          )
          .addOption(
            config
              .newOptionBuilder()
  ...
  switch (request.configParams.stats_type) {
    ...
    case 'categories': {
      ...
      fields
        ...
        .setDescription(
          'The percentage of send requests where the email was delivered to the recipients email provider.'
        )
    ...
    case 'mailbox_providers': {
      ...
      fields
        ...
        .setName('Open Rate')
        .setDescription(
          'The percentage of delivered emails where a recipient opened the email.'
        )
        .setType(cc.FieldType.PERCENT)
        .setGroup('Interactions')
      ...
      fields
        ...
        .setDescription(
          'The percentage of delivered emails where a recipient clicked a link in the email.'
        )
        .setType(cc.FieldType.PERCENT)
        .setGroup('Interactions')
      ...
      fields
        ...
        .setDescription(
          'The percentage of opened emails where a recipient clicked a link in the email.'
        )
        .setType(cc.FieldType.PERCENT)
        .setGroup('Interactions')
    ...
    default: {
      cc.newDebugError()
        .setText(
          'Unhandled stats_type "' + request.configParams.stats_type + '".'
        )
        .throwException();
      break;
    }
...
function getSendGridData(request) {
  ...
  var filters =
    request.configParams.filter !== undefined
      ? request.configParams.filter.split('\n')
      : undefined;
...
function buildURL(stats_type, aggregated_by, start_date, end_date, filters) {
  var url = ['https://api.sendgrid.com', 'v3'];
...
function getStatsJSON(url, apikey) {
  ...
  if (result.getResponseCode() == 200) {
    ...
    throwUserException(
      'Failed to retrieve SendGrid data. HTTP Error ' + result.getResponseCode()
    );
  }
}
