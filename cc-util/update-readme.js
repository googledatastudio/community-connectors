...
const getDeployments = async () => {
  const claspResults = await shellCommand('cd src && npx @google/clasp deployments')
  const [_numberOfDeployments, ...deployments] = claspResults.trim().split('\n')
  return deployments
}
...
const parseDeploymentId = (deploymentString) => {
  const deploymentIdReversed = deploymentString.substring(2).split('').reverse().join('')
  const atLocation = deploymentIdReversed.indexOf('@')
  const deploymentId = deploymentIdReversed.substring(atLocation + 2).split('').reverse().join('')
  return deploymentId
}
...
const updateReadme = async() => {
  ...
  const updatedContents = (await readFile(readmePath)).replace(/\[_latest deployment\]:.*/, `[_latest deployment]: ${latestDeploymentUrl}`)
