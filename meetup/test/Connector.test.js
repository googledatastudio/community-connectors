...
test('getCachedData with miss & do not add to cache because not enough responses url', () => {
  c.cache = initCache({});
...
test('getCachedData with miss & do not add to cache because missing url', () => {
  let response = [];
  for (let i = 0; i < 100; i++) {
...
test('rowify general_info', () => {
  const apiResult = {
    meta_category: {name: 'Tech', photo: {photo_link: 'tech_photo_link'}},
  ...
  const expected = [
    {
      values: [
        27009522,
        'GDG-Beja',
        '36.72999954223633, 9.1899995803833',
        'Bajah',
        'Tunisia',
        10,
        30,
...
test('getDataSchema correctly builds schema object with multiple fields requested', () => {
...
test('getDataSchema correctly builds schema object with only 1 field requested', () => {
...
test('getDataSchema throwsError when field not in the schema requested.', () => {
...
test('getDataCalls calls correct functions when passed general_info api type', () => {
...
test('getFetchOptions test', () => {
  ...
  expect(actual).toEqual({headers: {Authorization: 'Bearer accesstoken'}});
});
