...
test('build', () => {
  ...
  expect(oauth2Mock.setAuthorizationBaseUrlCalled).toBe(
    'https://accounts.spotify.com/authorize'
  );
  expect(oauth2Mock.setTokenUrlCalled).toBe(
    'https://accounts.spotify.com/api/token'
  );
  expect(oauth2Mock.setClientIdCalled).toBe('cid');
  expect(oauth2Mock.setClientSecretCalled).toBe('cs');
  expect(oauth2Mock.setPropertyStoreCalled).toBe(
    propertiesServiceMock.userProperties
  );
