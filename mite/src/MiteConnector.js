...
function getConfig(request) {
  ...
  config.newSelectSingle()
    ...
    .setHelpText('e.g. data that is retrieved from the Mite API will be cache in order to reduce API calls and increase performance')
    .setAllowOverride(true)
    .addOption(config.newOptionBuilder().setLabel('15 Minutes (Default)').setValue(15*60))
    .addOption(config.newOptionBuilder().setLabel('30 Minutes').setValue(30*60))
    .addOption(config.newOptionBuilder().setLabel('1 Hour').setValue(1*60*60))
    .addOption(config.newOptionBuilder().setLabel('3 Hours').setValue(3*60*60))
    .addOption(config.newOptionBuilder().setLabel('6 Hours').setValue(6*60*60))
  ...
  config.newSelectSingle()
    ...
    .addOption(config.newOptionBuilder().setLabel('Time entries').setValue(MiteTimeEntries.API))
    .addOption(config.newOptionBuilder().setLabel('Projects').setValue('projects'))
    .addOption(config.newOptionBuilder().setLabel('Users').setValue('users'))
    .addOption(config.newOptionBuilder().setLabel('Customers').setValue(MiteCustomers.API))
  ...
  if (params && params.api == MiteTimeEntries.API) {
    config.newTextInput()
      .setId('group_by')
      .setName('Group by')
      .setHelpText('e.g. free combination of user, customer, project, service and time (day, week, month or year) with values separated by comma. Leave blank for no grouping. Example: project, user, day')
      .setAllowOverride(false);
    ...
    .addOption(config.newOptionBuilder().setLabel('bon-billable').setValue(2))
...
function getApi(request) {
  ...
  switch(request.configParams.api) {
    ...
    case MiteTimeEntries.API:
      if (request.configParams.group_by && request.configParams.group_by.length > 0)
        return new MiteTimeEntriesGrouped(new MiteTimeEntries(), request.configParams.group_by);
      return new MiteTimeEntries();
    default:
      return new MiteTimeEntries();
  }
...
function getData(request) {
  ...
  var idHandler;
  if (filterPresent) {
    filter = new Filter(schema, request ? request.dimensionsFilters : undefined, Filter.AND);
  ...
  if (cacheTimespanSec > 0) {
    ...
    // see: https://developers.google.com/datastudio/connector/build#fetch_and_return_data_with_getdata
    cache = new DataCache(cacheService, 15 * 60, request.configParams.api, params);
    cached = true;
    data = cache.getJson();
  }
