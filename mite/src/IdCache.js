...
IdCache.prototype.buildParams = function(schema) {
  return schema.filter(field => field.hasOwnProperty('key')).reduce((mappings,field) => ({...mappings, [field.id]: field.key}), {});
}
...
IdCache.prototype.set = function(rows) {
  ...
  Object.keys(params).forEach(function(field) {
    ...
    values = rows.reduce((mappings,row) => ({...mappings, [row[field]]: row[key]}), values);
