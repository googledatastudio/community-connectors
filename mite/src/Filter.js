...
function Filter(schema, filter, filterType) {
  ...
  this.isPreFilter = function() {
    ...
    return (
      this.isDefaultPreFilter() ||
      this.isBooleanPreFilter() ||
      this.isTextPreFilter()
    );
  };
  ...
  this.preFilterDefault = function(params) {
    ...
    var values = [];
    if (params.hasOwnProperty(this.id)) values = params[this.id];
  ...
  this.resolveIdMappings = function(mappings, schema) {
    ...
    if (!idMappings) return;

    // map all filter values onto their corresponding values; e.g. 'user'='John Doe' => 'user_id'=1234
    var mappedValues = this.filter.values.reduce(
      (result, value) => [...result, idMappings[value]],
      []
    );
  ...
  this.preFilter = function(params) {
    ...
    else if (this.isDefaultPreFilter()) return this.preFilterDefault(params);
  ...
  this.match = function(row) {
    // no filters defined
    if (!this.filter || this.isPreFilter()) return;
