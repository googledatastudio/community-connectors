 ...
 */
function MiteInterface(dimensionsCallback, metricsCallback, tag, api, apiArchived) {
...
MiteInterface.prototype.getFields = function(request) {
  ...
  metrics.forEach(metric => fields.newMetric().setId(metric.id).setName(metric.name).setType(metric.type).setAggregation(metric.aggregation));
  ...
  if (request) {
    ...
    if (request.pagination) {
      ...
      params = {        
        ...
        page: Math.floor((request.pagination.rowCount + request.pagination.startRow) / request.pagination.rowCount)
      }
    } else
      params = {};
  } else {
