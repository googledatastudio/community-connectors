...
MiteTimeEntries.prototype.getFieldMappings = function() {
  return this.getSchema().filter(field=>field.hasOwnProperty('api')).reduce((mappings,field) => ({...mappings, [field.id]: field.api}), {});
}
...
MiteTimeEntries.prototype.getFields = function(request) {
  ...
  this.getDimensions().forEach(dimension => fields.newDimension().setId(dimension.id).setName(dimension.name).setType(dimension.type));
  ...
  if (request) {
    ...
    */
    ...
    if (request.pagination) {
      ...
      params['page'] = Math.floor((request.pagination.rowCount + request.pagination.startRow) / request.pagination.rowCount);
    }
...
MiteTimeEntries.prototype.getJson = function(credentials, params) { 
  ...
  var response = miteGet(credentials.domain, credentials.key, MiteTimeEntries.API, params);
